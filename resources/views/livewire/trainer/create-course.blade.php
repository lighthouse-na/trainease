...
new class extends Component {
    ...
    public function saveMaterial()
    {
        ...
        session()->flash('message', 'Course material added successfully!');
    }
        ...
        <div class="tab-content">
            <div x-show="$wire.activeTab === 'course'" class="p-4 rounded-lg bg-white" x-transition>
                ...
                <form>
                    ...
                    <x-button type="button" wire:click="saveCourse" class="bg-accent-content hover:bg-accent-content">
                        {{ $courseCreated ? 'Update Course' : 'Save Course' }}
                    </x-button>
                </form>
            </div>
                                ...
                                @forelse ($courseMaterials as $material)
                                    <div class="bg-white border rounded-lg   p-4">
                                        ...
                                        <div class="flex justify-between items-center">
                                            <a href="#"
                                                class="text-accent-content hover:underline text-sm">View</a>
                                            <span
                                                class="text-xs text-gray-400">{{ $material->created_at->format('M d, Y') }}</span>
                                        </div>
                                    </div>
            ...
            <div x-show="$wire.activeTab === 'quiz' && $wire.courseCreated" class="p-4 rounded-lg bg-white"
                x-transition>
                ...
                <form wire:submit.prevent="saveQuiz">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2" for="quizAttempts">Maximum Attempts</label>
                            <x-input id="quizAttempts" wire:model="quizAttempts" type="number" min="1" class="w-full" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2" for="passingScore">Passing Score (%)</label>
                            <x-input id="passingScore" wire:model="passingScore" type="number" min="0" max="100" class="w-full" />
                        </div>
                    </div>
            ...
            <div x-show="$wire.activeTab !== 'course' && !$wire.courseCreated"
                class="p-4 rounded-lg bg-white text-center" x-transition>
                <div class="py-8">
