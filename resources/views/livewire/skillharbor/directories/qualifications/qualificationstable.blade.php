...
<div>
    <x-skillharbor.layout heading="{{ __('Qualifications') }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach ($qualifications as $qualification)
                    ...
                    "
                    @contextmenu="contextMenuToggle(event)" class="relative z-50 flex w-full justify-between items-center cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 flex flex-col items-start justify-between gap-y-3 border rounded-lg p-4 dark:bg-gray-800 dark:border-gray-700">

                        <div class="flex flex-col space-y-1 text-sm text-gray-600 dark:text-gray-300 w-full">
                            <h4 class="text-lg font-semibold text-gray-800 dark:text-white mb-1">{{ $qualification->qualification_title }}</h4>
                        ...
                        <template x-teleport="body">
                            <div x-show="contextMenuOpen" @click.away="contextMenuOpen=false" x-ref="contextmenu" class="z-50 min-w-[8rem] text-neutral-800 dark:text-white rounded-md border border-neutral-200/70 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm fixed p-1 shadow-md w-64" x-cloak>
                                <div @click="contextMenuOpen=false; $wire.editQualification({{ $qualification->id }})" class="relative flex cursor-default select-none group items-center rounded px-2 py-1.5 hover:bg-neutral-100 dark:hover:bg-gray-700 outline-none pl-8">
                                ...
                                <div @click="contextMenuOpen=false; $wire.deleteQualification({{ $qualification->id }})" class="relative flex cursor-default select-none group items-center rounded px-2 py-1.5 hover:bg-neutral-100 dark:hover:bg-gray-700 outline-none pl-8 text-red-600 dark:text-red-400">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="absolute left-2 w-4 h-4" viewBox="0 0 20 20" fill="currentColor">
                                ...
                                <div @click="contextMenuOpen=false" class="relative flex cursor-default select-none group items-center rounded px-2 py-1.5 hover:bg-neutral-100 dark:hover:bg-gray-700 outline-none pl-8">
                                    <span>Cancel</span>
                                </div>
                            </div>
                        </template>
    ...
    <flux:modal name="qualificationModal" class="md:w-96">
        <div class="space-y-6 dark:bg-gray-800">
            <div>
            ...
            <flux:input wire:model="qualification" placeholder="Enter Qualification title" label="Qualification title" class="dark:bg-gray-700 dark:text-white">
            </flux:input>
